...
from scripts.build_natcom_polar_histogram_case import sanitize_public_json
...
class NatureCommunicationsPolarCaseTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.capabilities = json.loads(
            (GALLERY / "data" / "capabilities.json").read_text(encoding="utf-8")
        )
    ...
    def test_capability_atlas_links_rose_wind_to_this_case(self):
        capability = next(
            item for item in self.capabilities["capabilities"] if item["id"] == "rose-wind"
        )
        self.assertEqual(capability["engineStatus"], "coordinate_specific")
        self.assertEqual(capability["demoStatus"], "visible_geometry")
        self.assertEqual(capability["caseId"], CASE_ID)
        references = [item for item in self.capabilities["referenceCases"] if item["id"] == CASE_ID]
        self.assertEqual(len(references), 1)
        self.assertEqual(references[0]["evidenceStatus"], "candidate")
